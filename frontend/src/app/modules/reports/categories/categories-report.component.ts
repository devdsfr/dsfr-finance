...
@Component({
      ...
      <table class="table">
        ...
        <tbody>
          ...
          @for (row of rows(); track row.category) {
            <tr>
              <td>
                <span class="badge" [style.background]="row.color || '#2e7736'">{{ row.category }}</span>
              </td>
              <td>{{ row.type === 'expense' ? 'Despesa' : 'Receita' }}</td>
              <td>{{ row.count }}</td>
              <td>R$ {{ row.total | number:'1.2-2' }}</td>
              <td>
                <div class="bar-wrap"><div class="bar" [style.width]="pct(row.total) + '%'" [style.background]="row.color || '#2e7736'"></div></div>
  ...
  styles: [`
    ...
    :host-context([data-theme="dark"]) .table th { color: #8393ad !important; background: #1e2638 !important; }
    :host-context([data-theme="dark"]) .table td { border-top-color: #232d42 !important; }
    :host-context([data-theme="dark"]) .bar-wrap { background: #1e2638 !important; }
